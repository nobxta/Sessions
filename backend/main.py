...
def _start_cloudflare_tunnel():
    ...
    if not CF_BIN.exists():
        ...
        url = (
            ...
            if ("aarch64" in arch or "arm64" in arch) else
    ...
    if not CERT_FILE.exists():
        ...
        proc.wait()
        if not CERT_FILE.exists():
            print("[tunnel] Auth failed. Restart and authorize.")
            sys.exit(1)
    ...
    print(f"[tunnel] DNS: {(r.stdout or r.stderr).strip()}")
    ...
    time.sleep(3)
    print(f"\n[tunnel] Live → https://{TUNNEL_DOMAIN}\n")
    return proc

_tunnel_proc = _start_cloudflare_tunnel()
# ─────────────────────────────────────────────────────────────────────────────
