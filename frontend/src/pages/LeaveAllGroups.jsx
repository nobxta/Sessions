...
const LeaveAllGroups = () => {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        const errors = data.results.filter(r => !r.success).map(r => r.error);
        if (errors.length > 0) {
  ...
  const handleLeaveAll = () => {
    ...
    // Check if there are any groups to leave
    ...
    ws.onopen = () => {
      ws.send(JSON.stringify({
        ...
        temp_dirs: extractionData ? extractionData.flatMap(data => data.temp_dirs || []) : []
      }));
    };
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      {result.errors && result.errors.length > 0 && (
                        <div className="mt-2 text-xs text-red-400">
                          {result.errors.slice(0, 3).map((err, errIdx) => (
