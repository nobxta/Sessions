...
const JoinChatLists = () => {
  ...
  const [selectedFoldersToLeave, setSelectedFoldersToLeave] = useState({}); // {sessionIndex: [folderIds]}
  ...
  const handleScan = async () => {
    ...
    try {
      const tempDirs = extractionData.flatMap(data => data.temp_dirs || []);
  ...
  const handleToggleFolder = (sessionIndex, folderId) => {
  ...
  const handleClearSelection = (sessionIndex) => {
    setSelectedFoldersToLeave(prev => {
      const newState = { ...prev };
  ...
  const handleLeaveSelected = async () => {
    // #region agent log
    fetch('http://127.0.0.1:7244/ingest/ba597796-b716-4722-9dcf-137f94fd15bf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'JoinChatLists.jsx:148',message:'handleLeaveSelected called',data:{selectedFoldersToLeave,extractedSessionsCount:extractedSessions.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
    
    // Check if there are any selected folders to leave
    ...
    // Prepare sessions with premium status
    ...
    // #region agent log
    fetch('http://127.0.0.1:7244/ingest/ba597796-b716-4722-9dcf-137f94fd15bf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'JoinChatLists.jsx:176',message:'About to connect WebSocket',data:{sessionsCount:sessionsWithPremium.length,leaveConfigKeys:Object.keys(selectedFoldersToLeave),leaveConfig:selectedFoldersToLeave},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
    ...
    ws.onopen = () => {
      // #region agent log
      fetch('http://127.0.0.1:7244/ingest/ba597796-b716-4722-9dcf-137f94fd15bf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'JoinChatLists.jsx:ws.onopen',message:'WebSocket opened, sending data',data:{sessionsCount:sessionsWithPremium.length,leaveConfig:selectedFoldersToLeave,inviteLinksCount:0},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      ws.send(JSON.stringify({
        sessions: sessionsWithPremium,
        leave_config: selectedFoldersToLeave,
        invite_links: [], // Empty - only leaving, not joining
        temp_dirs: extractionData.flatMap(data => data.temp_dirs || [])
      }));
    };
    ...
    ws.onmessage = (event) => {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7244/ingest/ba597796-b716-4722-9dcf-137f94fd15bf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'JoinChatLists.jsx:handleLeaveSelected:ws.onmessage',message:'WebSocket message received',data:{type:message.type,hasIndex:message.index!==undefined,hasResult:message.result!==undefined,hasMessage:message.message!==undefined},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      ...
      switch (message.type) {
        ...
        case 'complete':
          ...
          // Refresh scan results after leaving
    ...
    ws.onerror = (err) => {
      // #region agent log
      fetch('http://127.0.0.1:7244/ingest/ba597796-b716-4722-9dcf-137f94fd15bf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'JoinChatLists.jsx:handleLeaveSelected:ws.onerror',message:'WebSocket error',data:{error:err.toString()},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
    ...
    ws.onclose = () => {
      // #region agent log
      fetch('http://127.0.0.1:7244/ingest/ba597796-b716-4722-9dcf-137f94fd15bf',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'JoinChatLists.jsx:handleLeaveSelected:ws.onclose',message:'WebSocket closed',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      wsRef.current = null;
    };
  };
  ...
  const handleRemoveInviteLink = (index) => {
  ...
  const handleStart = () => {
    ...
    // Validate link format if links are provided
    ...
    // Prepare leave config
    const leaveConfig = {};
    Object.keys(selectedFoldersToLeave).forEach(sessionIdx => {
      const folderIds = selectedFoldersToLeave[sessionIdx];
