...
const SessionMaker = () => {
  ...
  const [createdSessionPath, setCreatedSessionPath] = useState('');
  ...
  const resetFlow = () => {
    // Reset all state completely - no secrets remain in memory
    setCurrentStep(1);
    setPhoneNumber('');
    setPhoneCodeHash('');
    setSessionPath('');
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (data.needs_2fa) {
        setNeeds2FA(true);
        // Keep the session_path for 2FA step (if provided in response, use it; otherwise keep existing)
        ...
        setCreatedSessionPath(data.session_path);
        setCreatedFilename(data.filename);
        setCurrentStep(4);
  ...
  const handleDownloadSession = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/download-session', {
        ...
        body: JSON.stringify({
          session_path: createdSessionPath,
      ...
      // Note: Session file is deleted on backend after download
      // User must explicitly click "Create Another Session" to start new flow
    } catch (err) {
  ...
  const handleCreateAnotherSession = () => {
    // Explicit user action - reset all state cleanly
    resetFlow();
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {currentStep === 2 && (
          <div className="bg-white/[0.02] border border-white/10 rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              {/* Filename options (shown early for step 2) */}
        ...
        {/* Step 4: Session Save & Download - Result State Panel */}
        {currentStep === 4 && (
          <div className="bg-white/[0.02] border border-white/10 rounded-lg p-6">
            <div className="mb-4">
              <h2 className="text-xl font-semibold text-white mb-2">
              ...
              <p className="text-sm text-gray-400">
                Your session has been created successfully. You can download it now or start a new session creation.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
                ...
                <p className="text-sm text-gray-300">
                  Filename: <span className="font-mono text-white">{createdFilename}.session</span>
                </p>
              </div>
              ...
              <button
                ...
                className="w-full px-6 py-3 bg-green-500/20 hover:bg-green-500/30 border border-green-500/40 rounded-lg text-white font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isCreatingSession ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
              ...
              {/* Secondary Action: Create Another Session */}
              <button
