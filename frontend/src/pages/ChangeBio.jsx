...
const ChangeBio = () => {
  ...
  const handleFileSelect = async (selectedFiles) => {
    ...
    try {
      ...
      // Initialize bio inputs with empty strings
      const initialInputs = {};
      allSessions.forEach((session, index) => {
        initialInputs[index] = '';
      });
      setBioInputs(initialInputs);
  ...
  const fetchUserInfo = async (sessions) => {
    ...
    try {
      ...
      // Pre-fill inputs with current bio (if available from full profile)
      // Note: get_user_info doesn't return bio, so we'll fetch it separately if needed
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to update bios');
      }

      const data = await response.json();
      setResults(data.results || {});
    } catch (err) {
      setError(err.message || 'Failed to update bios');
    } finally {
      setIsUpdating(false);
    }
  };
        ...
        <div className="mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              Upload session files and update bio for all accounts
            </p>
          </div>
        </div>
        ...
        {extractedSessions.length > 0 && (
          <div className="mb-8">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        ...
                        <textarea
                          value={bioInputs[index] || ''}
                          onChange={(e) => handleBioChange(index, e.target.value)}
                          placeholder="Enter new bio (max 70 characters)"
                          disabled={isUpdating || isSuccess}
                          rows={3}
                          maxLength={70}
                          className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-white/30 disabled:opacity-50 resize-none"
                        />
                        <div className="flex items-center justify-between mt-1">
                          <div></div>
                          <span className="text-xs text-gray-500">
                            {(bioInputs[index] || '').length} / 70
                          </span>
                        </div>
                        {hasResult && !isSuccess && (
            ...
            {isUpdating && (
              <div className="mt-4 flex items-center gap-2 text-sm text-gray-400">
                <Loader2 className="w-4 h-4 animate-spin" />
                <span>Updating bios in parallel...</span>
              </div>
            )}
